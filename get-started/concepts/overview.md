...
For a simplified illustration of the Access Policy evaluation flow, see [Evaluation flow: how Aembit grants access](../how-aembit-works.md#access-policy-flow-putting-it-all-together).
...
Instead of Client Workloads managing and presenting a long-lived secret for authentication, Aembit uses [Trust Providers](trust-providers.md). Trust Providers cryptographically verify the identity of Client Workloads attempting to access target Server Workloads. Trust Providers verify a Client Workload’s identity using evidence obtained directly from its runtime environment—also known as workload attestation.
...
During [Access Policy evaluation](../how-aembit-works.md#access-policy-flow-putting-it-all-together), Aembit Cloud first matches the Client and Server Workloads to an Access Policy *and* verifies the Client Workload’s identity. Aembit Cloud then explicitly evaluates all associated Access Conditions. Only if all Access Conditions pass, along with the Client Workload’s identity check, does the Access Policy grant access and trigger the Credential Provider.
...
Aembit uses [Content Security Providers](content-security.md) to govern the Model Context Protocol (MCP) tool traffic that flows through Aembit’s MCP Identity Gateway. You add a provider to an Access Policy through its optional Content Security component. Each provider evaluates the MCP tool messages the MCP Identity Gateway proxies, and Aembit applies the provider’s decision before the message continues.

A provider acts on what an agent discovers, the set of MCP tools an MCP server advertises, and on what an agent calls.

CrowdStrike AI Detection and Response (AIDR) inspects MCP content in real time and returns a verdict that Aembit enforces.

![](https://docs.aembit.io/aembit-icons/lightbulb-light.svg)

[More on Content Security](content-security.md)See Core Concepts

→

![](https://docs.aembit.io/aembit-icons/gears-light.svg)

[Set up Content Security](../../user-guide/access-policies/content-security/overview.md)See the Aembit User Guide
...
You can [administer Aembit Cloud](administration.md) through your unique, and isolated Aembit Tenant to define access rules, configure trust and credential sources, and monitor access events.
...
>
> Crucially, Aembit functions purely as a control plane; it doesn’t process or log any actual data from your workloads, only metadata related to access control. The one exception is the [CrowdStrike AIDR](../../user-guide/access-policies/content-security/crowdstrike-aidr/overview.md) Content Security Provider, where Aembit sends inspected MCP content to CrowdStrike. Even then, Aembit’s own events record only metadata, never the inspected content.
...
This helps reduce manual steps, prevent configuration drift, and ensure your access policies are reproducible and reviewable.
