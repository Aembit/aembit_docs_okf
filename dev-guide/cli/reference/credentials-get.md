...
resource: https://docs.aembit.io/dev-guide/cli/reference/credentials-get/
interface: cli
tags: ["reference", "cli"]
...
```plaintext
aembit:<stack>:<tenantId>:identity:<identityType>:<trustProviderId>
```

Detailed example

If your Aembit Tenant is hosted on the `useast2` stack, has a tenant ID of `a12bc3`, and uses a GitLab identity type with a trust provider ID of `63ab7be6-9785-4a14-be1c-2acf0253070b`, your client ID would look like this:

```shell
aembit:useast2:a12bc3:identity:gitlab_idtoken:63ab7be6-9785-4a14-be1c-2acf0253070b
```

* `useast2` is the `<stack>` that hosts your Aembit Tenant
...
>
> Which names your Aembit Tenant accepts depends on the Credential Provider that the matched Access Policy maps, so check [Credential name support](../credential-names.md) before you choose a name.
>
...
**Default** - not set\
**Env var**: [AEMBIT\_CLIENT\_ID](../../../reference/edge-components/edge-component-env-vars.md#aembit_client_id)\
...
* The key must remain **unencrypted** at rest. Passphrase-protected keys aren’t supported.
...
Aembit Cloud signs a CSR that uses any of the following key algorithms: RSA (2048 or 4096 bits) or ECDSA (P-256 or P-384). The CLI rejects keys outside these ranges. Choose an algorithm and key size that meet your requirements within those bounds.

### `--client-workload-id`

**Default** - not set\
**Env var**: [CLIENT\_WORKLOAD\_ID](../../../reference/edge-components/edge-component-env-vars.md#client_workload_id)\
**Description** - The Aembit Client ID of the Client Workload you want Aembit to match, used to identify a specific Client Workload when multiple workloads share the same Trust Provider. Use this when your environment has two or more Client Workloads that would otherwise match the same Trust Provider attestation.\
**Example** - `7e75e718-7634-480b-9f7b-a07bb5a4f11d`

Despite its name, this flag doesn’t take the Client Workload’s own resource ID. It takes the value Aembit generates when you configure **Aembit Client ID** as the Client Workload’s Client Identification method. To obtain it, see [Aembit Client ID](../../../user-guide/access-policies/client-workloads/identification/aembit-client-id.md).

The target Client Workload must have **Aembit Client ID** configured as a Client Identification method. Aembit matches this value only against that identifier type, so a workload identified solely by another method, such as hostname or source IP, never matches this flag.

> **Three similarly named identifiers**
>
> The Aembit Client ID and the Client Workload’s resource ID use the same UUID format, and supplying the wrong one causes Access Policy matching to fail. If `--client-workload-id` fails to match, confirm you copied the generated **Aembit Client ID** value rather than the Client Workload’s resource ID.
>
> Aembit CLI error messages report the resource ID, not the Aembit Client ID. A message such as `Matched client workload ID: [client-workload-id].` identifies which workload Aembit matched, so use that value to look the workload up in your tenant rather than passing it to this flag.
>
> A saved Aembit Client ID can also go stale, because Aembit generates a new value if someone removes the identifier and adds it back.
>
> For a side-by-side comparison of all three identifiers and an explanation of [why a Client Workload has two UUID values](../../../reference/identifiers.md#why-a-client-workload-has-two-uuid-values), see the [Aembit identifier reference](../../../reference/identifiers.md).
...
**Examples** - `MY_TOKEN`, `USERNAME,PASSWORD`, `AWS_ACCESS_KEY_ID,AWS_SECRET_ACCESS_KEY,AWS_SESSION_TOKEN`

> **Not every Credential Provider accepts a name of your choosing**
>
> Your Aembit Tenant returns a value only for the names the matched Credential Provider recognizes. Some Credential Providers recognize a fixed set of names instead of yours. Others return all their values as one JSON document unless you pass the exact names they define. Requesting a name that the Credential Provider doesn’t recognize returns:
>
> ```text
> Credential(s) not returned by tenant: <YOUR_CUSTOM_NAME>
> ```
>
> See [Credential name support](../credential-names.md) for the Credential Providers that limit the names you can use, and for how to use a different name for a credential.
...
**Description** - Tells Aembit CLI which environment-specific data to gather when it attests to Aembit Cloud. Set this flag to match where the CLI runs so Aembit CLI can locate the Instance Metadata Service (IMDS) endpoint, projected service account token, or container metadata it needs.
...
**Env var**: [AEMBIT\_LOG\_LEVEL](../../../reference/edge-components/edge-component-env-vars.md#aembit_log_level)\
**Description** - The log level to use for the Aembit CLI. This controls the verbosity of the output from the CLI.

### `--output-format`

**Default** - `sh-export` on Linux and macOS, and `powershell-env` on Windows\
...
**Default** - not set\
**Env var**: [AEMBIT\_RESOURCE\_SET\_ID](../../../reference/edge-components/edge-component-env-vars.md#aembit_resource_set_id)\
...
```shell
# Get credentials with a custom name
aembit credentials get \
  --client-id aembit:useast2:a12bc3:identity:github_idtoken:63ab7be6-9785-4a14-be1c-2acf0253070b \
  --server-workload-host example.com \
  --server-workload-port 443 \
