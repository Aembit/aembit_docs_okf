...
The same field also appears on the corresponding [MCP Workload Events](workload-events.md), so you can take any context ID from the chain to trace a request across both surfaces. See [Trace a request with the authorization chain](../troubleshooting/mcp-ai-iam.md#trace-a-request-with-the-authorization-chain).
...
[Section titled “Access Request events”](#access-request-events)
...
[Section titled “Authorization Failure”](#authorization-failure)
...
[Section titled “Access Discovery events”](#access-discovery-events)
...
Access Discovery events are emitted alongside the standard `access.request` and `access.authorization` events when discovery applies, such as during Model Context Protocol (MCP) Identity Gateway authorization where a single client request can match multiple candidate Server Workloads.

When no workload or policy matches the request, the discovery event has severity `Error` and lists each candidate that Aembit Cloud evaluated along with the reason it was excluded.
...
[Section titled “Access Credential events”](#access-credential-events)
...
[Section titled “Retrieving Access Authorization Event data”](#retrieving-access-authorization-event-data)
...
[Section titled “Viewing Access Authorization event data”](#viewing-access-authorization-event-data)
...
[Section titled “Access authorization event example”](#access-authorization-event-example)
...
[Section titled “Trust Provider attestation failure example”](#trust-provider-attestation-failure-example)
...
* **Trust Providers** - The Trust Providers assigned to the Access Policy at the time of evaluation.
  ...
  * If a `reason` value is returned, refer to the [Authorization Failure](#authorization-failure) section on this page for more information.
...
If a `reason` value is returned, refer to the [Credential Failure](#credential-failure) section on this page for more information.
