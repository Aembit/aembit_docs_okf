...
resource: https://docs.aembit.io/user-guide/deploy-install/mcp-authorization-server/concepts-mcp-auth-server/
...
This page covers the key concepts you need to understand when working with the Aembit Model Context Protocol (MCP) Authorization Server. For setup instructions, see [Set up the MCP Authorization Server](../../access-policies/mcp-authorization-server/setup-mcp-auth-server.md).
...
* **Trust Provider** - Validates user identity during the authorization flow. For MCP with human authentication, the OIDC ID Token Trust Provider matches claims (issuer, audience, subject) from your identity provider to verify the user. When [Enforce SSO is off](#how-no-user-auth-authorization-works) on the Client Workload, the policy needs no Trust Provider.
...
For step-by-step configuration, see [Set up the MCP Authorization Server](../../access-policies/mcp-authorization-server/setup-mcp-auth-server.md).
...
* OIDC
  ...
  ![MCP Authorization Server architecture with OIDC identity provider](https://docs.aembit.io/d2/docs/user-guide/deploy-install/mcp-authorization-server/concepts-mcp-auth-server-0.svg)

* SAML

  ![MCP Authorization Server architecture with SAML identity provider](https://docs.aembit.io/d2/docs/user-guide/deploy-install/mcp-authorization-server/concepts-mcp-auth-server-1.svg)

* No user auth

  ![MCP Authorization Server architecture without user authentication (Enforce SSO disabled)](https://docs.aembit.io/d2/docs/user-guide/deploy-install/mcp-authorization-server/concepts-mcp-auth-server-2.svg)
...
>
> When Enforce SSO is off on the Client Workload, the MCP Authorization Server skips user authentication entirely. The flow includes no browser redirect, no identity provider interaction, and no Trust Provider validation. The Credential Provider still generates the access token, and Access Conditions still apply.
...
To configure a Client Workload that uses CIMD, see [CIMD Client ID](../../access-policies/client-workloads/identification/cimd-client-id.md).
...
Configure the credential type in your Access Policy’s Credential Provider settings. See [Credential Providers](../../access-policies/credential-providers/overview.md) for available options.
...
To configure refresh token support, see [OIDC ID Token](../../access-policies/credential-providers/oidc-id-token.md) or [Aembit Access Token](../../access-policies/credential-providers/aembit-access-token.md).
...
![URL configuration alignment between MCP client, MCP server, and Aembit](https://docs.aembit.io/d2/docs/user-guide/deploy-install/mcp-authorization-server/concepts-mcp-auth-server-3.svg)
...
* [MCP Identity Gateway concepts](../mcp-identity-gateway/concepts-mcp-gateway.md) - How left-side and right-side auth apply in the Gateway context
