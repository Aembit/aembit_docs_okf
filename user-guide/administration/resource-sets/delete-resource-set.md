...
>
> Deleting a Resource Set also deletes all entities associated with it. Any Agent Proxy or Edge components configured to use the Resource Set stop functioning after the operation. You can’t undo this action.
...
4. Start the deletion in one of two ways:
   ...
   * Or, select the custom Resource Set to open its details panel, then click **Delete**.
   ...
   Aembit displays the **Delete Resource Set** confirmation dialog, which lists everything that Aembit deletes.
...
* **Aembit decommissions managed-account integrations.** If the Resource Set contains a managed-account integration (such as a [Managed GitLab Account](../../access-policies/credential-providers/managed-gitlab-account.md)), Aembit decommissions the integration and deletes the associated managed account.
...
* **Aembit leaves no data behind.** Aembit removes the Resource Set and all its associated entities, leaving no orphaned data in the UI or API.
