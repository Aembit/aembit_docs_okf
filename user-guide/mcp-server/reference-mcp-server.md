...
resource: https://docs.aembit.io/user-guide/mcp-server/reference-mcp-server/
interface: mcp
tags: ["mcp-server"]
...
See also [Audit Logs](../audit-report/audit-logs.md)
...
See also [Audit and Report](../audit-report/overview.md)
...
See also [Workload Events](../audit-report/workload-events/overview.md)
...
Valid values for the `category` filter in `get_audit_logs`. Each category corresponds to a type of activity logged in the Aembit Admin UI. See [Audit Logs](../audit-report/audit-logs.md) for descriptions of each category.
...
Valid values for the `eventType` filter in `get_auth_events`. Each type represents a step in Aembit’s access authorization flow. See [Access Authorization Events](../audit-report/access-authorization-events.md) for the full event schema.
